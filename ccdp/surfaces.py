...
# ---- human observability (lazy VNC) ----
def start_vnc(key):
    rec = _require(key)
    if rec.get("vnc_port") and util.pid_alive(rec.get("vnc_pid")):
        return rec["vnc_port"]
...
def stop_vnc(key):
    rec = registry.get(key)
    if rec and rec.get("vnc_pid"):
        util.terminate(rec["vnc_pid"])
        registry.upsert(key, dict(vnc_port=None, vnc_pid=None))
...
def close(key, *, purge_profile=False):
    ...
    if not rec:
        return
    for p in ("vnc_pid", "chrome_pid", "xvfb_pid"):
...
def pss_mb(rec):
    total = 0
    pids = set()
    for base in ("xvfb_pid", "chrome_pid", "vnc_pid"):
        if rec.get(base):
